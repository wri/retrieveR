...
cat("\n")
country <- as.character(readline(prompt= "Country? "))
cat("\n")
query <- as.character(readline(prompt = "Query? "))
cat("\n")
...
create_report <- function(target_country) {
  ...
  topn <- data %>%
    arrange(desc(results)) %>%
    select(name, country, legible, results, sentences, page) %>%
...
similars <- unname(unlist(wv1 %>% closest_to(as.matrix(query_vector), n=50) %>% select(word)))
...
print_margin <- function(thresh, inp_country) {
...
ggplot(data=testing, aes(x=reorder(name,density2), y=yes))+
  ...
  theme(line = element_blank(), rect = element_blank(), axis.title = element_blank(), 
        ...
        strip.text = element_text(size = rel(0.7)), axis.text.x=element_blank(), plot.margin = unit(c(0, 
